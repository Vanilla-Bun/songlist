...
function applySearchPlaceholder(allSongs, locale = appState?.ui?.locale ?? DEFAULTLOCALE) {
	const input = document.getElementById("searchInput");
	if (!input) return;
...
function normalizeForSearch(value) {
	return (value ?? "")
		.toString()
		.normalize("NFD")
		.replace(/макsим/gi, "максим")
		.replace(/m[åa]neskin/gi, "maneskin")
		.replace(/zdob\s+si\s+zdub/gi, "zdob si zdub")
		.replace(/zdob\s+și\s+zdub/gi, "zdob si zdub")
		.replace(/mylene/gi, "mylene")
		.replace(/alizee/gi, "alizee")
		.replace(/fleur/gi, "fleur")
		.replace(/ap\$ent/gi, "apsent")
		.replace(/[\u0300-\u036f]/g, "")
		.replace(/ё/gi, "е")
		.replace(/і/gi, "и")
		.replace(/ї/gi, "и")
...
function buildSearchVariants(value) {
	...
	const layoutFixed = normalizeForSearch(fixKeyboardLayout(raw));
	return Array.from(new Set([direct, layoutFixed].filter(Boolean)));
}

function enrichWithNormalizedFields(rawSongs) {
	return rawSongs.map((song) => {
		const titleN = normalize(song.title);
		const artistN = normalize(song.artist);

		const visibleAttributes = (song.attributes ?? "")
			.toString()
			.split(",")
			.map((s) => s.trim())
			.filter(Boolean)
			.filter((attr) => isAllowedTagAttribute(attr));

		const attrsN = visibleAttributes.map(normalize).join(", ");

		const searchBlob = [
			song.title,
			song.artist,
			visibleAttributes.join(", "),
			...buildSearchVariants(song.title),
			...buildSearchVariants(song.artist),
			...visibleAttributes.flatMap((attr) => buildSearchVariants(attr))
		].join(" ");
		...
		return {
			...
			searchBlob: normalizeForSearch(searchBlob)
		};
	});
}
...
function applySearch(songs, searchState) {
	...
	const queryVariants = buildSearchVariants(trimmed);
	const wordsVariants = queryVariants
		.map((variant) => variant.split(" ").filter(Boolean))
		.filter((words) => words.length > 0);

	if (wordsVariants.length === 0) return songs;

	return songs.filter((song) => {
		const haystack = song.searchBlob || "";
		return wordsVariants.some((words) => words.every((word) => haystack.includes(word)));
	});
}
...
function deriveVisibleSongs(state) {
	const filteredSongs = state.data.filteredSongs;
...
function recalculateFiltered(state) {
	...
	return { ...state, data: { ...state.data, filteredSongs: result } };
}
...
function handleModalCloseSideEffects(prevState, nextState) {
	...
	teardownModalFocus(modalRoot); // СНАЧАЛА убираем фокус
...
function getInitialState() {
	...
	return {
		data: { allSongs: [], filteredSongs: [], sha: null, isLoading: false, error: null },
		ui: {
...
const actions = {
	...
	csvLoadStart: () => ({ type: "CSV_LOAD_START" }),
	csvLoadSuccess: (songs, sha) => ({ type: "CSV_LOAD_SUCCESS", payload: { songs, sha: sha ?? null } }),
	csvLoadError: (error) => ({ type: "CSV_LOAD_ERROR", payload: { error: String(error ?? "Unknown error") } }),
...
function clampPage(page, stateForClamp) {
	const derived = deriveVisibleSongs(stateForClamp);
...
function resolveSongFromModalPayload(state, payload) {
	...
	if (typeof value === "number" && Number.isFinite(value)) {
		...
		return derived.visibleSongs[value] || null;
	}
	return null;
}
...
function runModalCardLoadEffects(action, nextState) {
	...
	dispatch(actions.modalCardLoadStart());

	const decoded = decodeCardNumber(encodedCard);
	if (decoded) {
		dispatch(actions.modalCardLoadSuccess(decoded));
		return;
	}

	const locale = nextState?.ui?.locale ?? DEFAULT_LOCALE;
	dispatch(actions.modalCardLoadError(t("modal.cardLoadFailed", locale)));
}
...
function runCsvEffects(action, nextState) {
	if (action.type !== "CSV_LOAD_SUCCESS") return;
...
async function handleSideEffects(action, prevState, nextState) {
	...
	runCsvEffects(action, nextState);
}
...
function dispatch(action) {
	...
	handleSideEffects(action, prevState, nextState);
}
...
const SettingsMenu = {
	...
	init(rootElement, api) {
		...
		this.panelElement.setAttribute(
			"aria-hidden",
			this.panelElement.hidden ? "true" : "false"
		);
	...
	close(returnFocus = false) {
		...
		if (returnFocus && typeof this.buttonElement.focus === "function") {
			setTimeout(() => this.buttonElement.focus(), 0);
		}
	},
	...
	const locale = state && state.locale ? state.locale : appState?.ui?.locale ?? DEFAULTLOCALE;
...
const SongCards = {
	...
	render(visibleSongs) {
		...
		if (!this.cachedVisibleSongs.length) {
			...
			const title = document.createElement("div");
			title.className = "song-cards-emptytitle";
			title.textContent = t("table.emptyTitle", locale);
		...
		for (let i = 0; i < this.cachedVisibleSongs.length; i++) {
			...
			const title = document.createElement("div");
			title.className = "song-cardtitle";
			title.textContent = song.title;

			const artist = document.createElement("div");
			artist.className = "song-cardartist";
			...
			btn.setAttribute(
				"aria-label",
				t("table.requestAria", locale, { artist: song.artist, title: song.title })
			);
...
function applyStaticI18n(state) {
	const locale = state && state.ui && state.ui.locale ? state.ui.locale : DEFAULTLOCALE;
...
async function checkAndLoadCSV() {
	...
	try {
		...
		const storedSha = localStorage.getItem("songlist-sha");
		const storedData = localStorage.getItem("songlist-data");

		const commitResponse = await fetch(commitUrl);
		if (!commitResponse.ok) throw new Error("Commit fetch failed");

		const commitData = await commitResponse.json();
		const latestSha = commitData[0]?.sha || null;

		if (storedSha && latestSha && storedSha === latestSha && storedData) {
			parseAndDispatchCSV(storedData, latestSha);
			return;
		}
		...
		if (latestSha) localStorage.setItem("songlist-sha", latestSha);
		localStorage.setItem("songlist-data", csvText);

		parseAndDispatchCSV(csvText, latestSha);
	} catch (error) {
		const storedData = localStorage.getItem("songlist-data");
		if (storedData) parseAndDispatchCSV(storedData, null);
		else dispatch(actions.csvLoadError(String(error && error.message ? error.message : error)));
	}
}

function parseAndDispatchCSV(csvText, sha) {
	Papa.parse(csvText, {
		header: true,
		skipEmptyLines: true,
		complete: (results) => {
			const rawSongs = results.data.filter((row) => row && row.active === "true");
			const enrichedSongs = enrichWithNormalizedFields(rawSongs);
			dispatch(actions.csvLoadSuccess(enrichedSongs, sha));
		}
	});
}

// -------------------------
// Sticky offsets helpers
// -------------------------
...
updateStickyOffsets();
window.addEventListener("resize", debounce(scheduleStickyOffsetsUpdate, 100));
