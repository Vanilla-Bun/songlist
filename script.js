...
function readFiltersFromUrl() {
	try {
		...
		const labels = params.getAll("f");
		if (!labels.length) return [];
		return labels
			.map(label => Object.entries(FILTER_DEFS)
				.find(([, def]) => def.allow && def.label === label)?.[0])
			.filter(Boolean);
	} catch {
		return [];
	}
...
function syncFiltersToUrl(activeKeys) {
	try {
		const params = new URLSearchParams();
		activeKeys.forEach(key => {
			const def = FILTER_DEFS[key];
			if (def && def.allow && def.label) params.append("f", def.label);
		});
...
function buildSearchVariants(value) {
	...
	if (shouldUseLayoutVariant(raw)) {
		const fixed = normalize(fixKeyboardLayout(raw.toLowerCase()));
			...
			if (toolbar) {
				const ro = new ResizeObserver(() => {
					...
					requestAnimationFrame(() => {
						balanceFilterRows(document.getElementById("filtersBar"));
					});
				});
				ro.observe(toolbar);
			}
...
function getInitialState() {
	...
	return {
		...
		ui: {
			...
			search: { query: "" },
			filters: { activeKeys: readFiltersFromUrl() },
...
// Reducer
// -------------------------
function clampPage(page, stateForClamp, prefiltered) {
	const filtered = prefiltered !== undefined ? prefiltered : selectFilteredSongs(stateForClamp);
...
function updateState(state, action) {
	switch (action.type) {
		...
		case "DATA_LOAD_SUCCESS": {
			...
			const filteredForClamp = selectFilteredSongs(next);
			const clamped = clampPage(next.ui.pagination.currentPage, next, filteredForClamp);
...
function persistUiSettings(action, nextState) {
	...
	if (action.type === "FILTER_TOGGLE" || action.type === "FILTER_CLEAR_ALL") {
		syncFiltersToUrl(nextState.ui.filters.activeKeys);
	}
}
...
function renderFilterButtons(container) {
	...
	Object.entries(FILTER_DEFS)
		...
		.forEach(([key, def]) => {
			...
			if (def.highlight) btn.dataset.highlight = def.highlight;
...
const _chipTextWidthCache = new Map();
const CHIP_PADDING_H    = 18;
const CHIP_ICON_W       = 17;
const CHIP_SUBPX_BUFFER =  4;

function measureChipWidth(chip) {
	const labelEl  = chip.querySelector("span:not(.icon)");
	const iconEl   = chip.querySelector(".icon");
	const labelText = labelEl ? labelEl.textContent : chip.textContent;
	const iconW    = iconEl ? CHIP_ICON_W : 0;

	let textW;
	if (_chipMeasureCtx) {
		if (_chipTextWidthCache.has(labelText)) {
			textW = _chipTextWidthCache.get(labelText);
		} else {
			textW = _chipMeasureCtx.measureText(labelText).width;
			_chipTextWidthCache.set(labelText, textW);
		}
	} else {
		textW = labelText.length * 7;
	}

	return Math.ceil(iconW + textW + CHIP_PADDING_H + CHIP_SUBPX_BUFFER);
}

function balanceFilterRows(container) {
	if (!container) return;

	const chips = Array.from(container.querySelectorAll(".chip"));
	if (!chips.length) return;

	container.querySelectorAll(".filters-row").forEach(r => {
		r.querySelectorAll(".chip").forEach(c => container.appendChild(c));
		r.remove();
	});
	...
	const GAP = 5;
	const widths = chips.map(measureChipWidth);
	const totalW = widths.reduce((s, w) => s + w, 0) + GAP * (chips.length - 1);

	const rowCount = Math.max(1, Math.ceil(totalW / containerW));
	...
	for (let i = 0; i < chips.length; i++) {
		const w = widths[i];
		const addW = curW > 0 ? curW + GAP + w : w;

		if (curRow.length > 0 && addW > targetW * 1.1 && rows.length < rowCount - 1) {
			rows.push(curRow);
			curRow = [chips[i]];
			curW = w;
		} else {
...
const SongTable = {
	...
	updateSongRow(tr, song, index) {
		...
		const tdTitle   = tr.querySelector(".song-tablecell--title");
		const tdArtist  = tr.querySelector(".song-tablecell:not(.song-tablecell--title):not(.song-tablecell--tags):not(.song-tablecell--actions)");
		const tdTags    = tr.querySelector(".song-tablecell--tags");
		const tdActions = tr.querySelector(".song-tablecell--actions");
...
function initApp() {
	...
	requestAnimationFrame(() => balanceFilterRows(filtersRoot));
